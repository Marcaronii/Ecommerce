...
export default function Checkout() {
  ...
  if (cart.length === 0 && !isSubmitted) {
    return (
      <div className="max-w-7xl mx-auto px-4 py-16 text-center">
        ...
        <button onClick={() => navigate('/')} className="text-blue-600 hover:underline text-lg">Return to Home</button>
      </div>
    );
  }
  ...
  const handleChange = (e) => {
    ...
    if (name === 'phone') {
      sanitizedValue = value.replace(/[^\d+\s-]/g, '').slice(0, MAX_LENGTHS.phone);
    }
  ...
  if (isSubmitted) {
    return (
      <div className="max-w-7xl mx-auto px-4 py-16 text-center flex flex-col items-center">
        ...
        <p className="text-gray-600 mb-8 max-w-md">Thank you for your purchase. Your order will be delivered soon to {formData.address}.</p>
        <button onClick={() => navigate('/')} className="bg-blue-600 text-white px-6 py-3 rounded-md text-lg font-semibold hover:bg-blue-700 transition-colors">
          Continue Shopping
        </button>
      </div>
    );
